...
runner_group = project.main_group.groups.find do |g|
  g.path.to_s == 'Runner' || g.name.to_s == 'Runner'
end
...
def main() -> None:
    if not SOURCE.is_file():
        raise SystemExit(f"Firebase iOS config missing: {SOURCE}")
    if not PROJECT.is_dir():
        raise SystemExit(f"{PROJECT} missing — run flutter create first")
    ...
    with tempfile.NamedTemporaryFile(
        ...
        f.write(RUBY_SCRIPT)
        ruby_file = Path(f.name)

    try:
        result = subprocess.run(
            ["ruby", str(ruby_file), str(PROJECT), "GoogleService-Info.plist"],
            cwd=ROOT,
            text=True,
            capture_output=True,
        )
        if result.returncode != 0:
            ...
            raise SystemExit(
                "Could not register GoogleService-Info.plist with Xcode project"
            )
        if result.stdout.strip():
            print(result.stdout.strip())
    finally:
        ruby_file.unlink(missing_ok=True)
